...
Adafruit_BNO055 IMU_hand(56,0x29);//IMUを扱うインスタンスを生成
...
void setup(){
  ...
  bool hand_conection = IMU_hand.begin();
  ...
  if(!hand_conection){
    Serial2.println("no BNO055(hand) detected");
    while(1){}
  }else{
    IMU_hand.setExtCrystalUse(true);
    Serial2.println("IMU_hand setting complete");
  }
  ...
  IMU_hand.setAxisRemap(Adafruit_BNO055::REMAP_CONFIG_P2);
  IMU_hand.setAxisSign(Adafruit_BNO055::REMAP_SIGN_P2);


  uint8_t a_mag_calib=0;
  uint8_t a_gyro_calib=0;
  uint8_t a_sys_calib=0;
  uint8_t a_accel_calib=0;

  uint8_t h_mag_calib=0;
  uint8_t h_gyro_calib=0;
  uint8_t h_sys_calib=0;
  uint8_t h_accel_calib=0;

  do
  {
    Serial2.print("IMU_arm calibrating...");
    Serial2.print(a_mag_calib);
    Serial2.print("of3   ");
    Serial2.print("IMU_hand calibrating...");
    Serial2.print(h_mag_calib);
    Serial2.println("of3");
    IMU_arm.getCalibration(&a_sys_calib,&a_gyro_calib,&a_accel_calib,&a_mag_calib);
    IMU_arm.getCalibration(&h_sys_calib,&h_gyro_calib,&h_accel_calib,&h_mag_calib);
  } while (a_mag_calib != 3 || h_mag_calib != 3);
...
void loop(){
  ...
  if(SoftSerial_str == "1"){
    ...
    result = 0;
  }else if(SoftSerial_str == "ff"){
    control = FF;
    result = judge_1to2 + 0.1;
  }else if(SoftSerial_str == "fb"){
    control = FB;
    result = judge_1to2 + 0.1;
  }
  ...
  case 1/*義手を閉じて水平制御OFF*/:{
    SerialServo.RegWritePosEx(4,2300,servo_v,servo_a);//指を閉じる
    SerialServo.RegWritePosEx(5,1600,servo_v,servo_a);
    SerialServo.RegWriteAction();
    move_time(2600,2047,servo_v,servo_a);
